...
const Slider = ({
  ...
  function applySelectedAttraction() {
    ...
    const containerWidth = slider.current.clientWidth;
    const cellWidth = slides.current[selectedIndex.current].cells[0].element.clientWidth;

    // ✅ Adjust position to center the cell
    let centeredPosition;
    if (isWrapping.current) {
      centeredPosition = -slides.current[selectedIndex.current].target + ((containerWidth - cellWidth) / 2);
    } else {
      centeredPosition = -slides.current[selectedIndex.current].target;
    }
  ...
  useEffect(() => {
    ...
    function handleResize() {
      // If selectedIndex is out of bounds, center the first cell
      if (el && !slides.current[selectedIndex.current] && slides.current[0]) {
