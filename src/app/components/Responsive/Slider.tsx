...
const Slider = ({
  ...
  useEffect(() => {
    ...
    function handleResize() {
      ...
      const wrap = childCount - 2 > images;
  ...
  }, [firstChildWidth, clonedChildren.length, slidesState.length, isMeasured]);
  ...
  function applySelectedAttraction() {
    ...
    const containerWidth = slider.current.clientWidth;
    const contentWidth   = sliderWidth.current || slider.current.scrollWidth;

    if (sliderWidth.current <= slider.current.clientWidth) {
      distance = (containerWidth - contentWidth) / 2 - sliderX.current;
    }
      ...
      if (!isWrapping.current) {
        ...
        const idx = Math.round(
          Math.abs(currentPosition) /
            (sliderWidth.current / slides.current.length)
        );
  ...
  return (
    <div ref={sliderContainer} className={styles.slider_container} style={{ position: 'relative', height: imageCount > 2 ? `${(sliderHeight === 0 ? 375 : sliderHeight) + 6}px` : `${(sliderHeight === 0 ? 375 : sliderHeight)}px`, backgroundColor: '#f8f9fa', zIndex: 1 }}>
      ...
      }`} style={{ position: 'relative', height: imageCount > 2 ? `${(sliderHeight === 0 ? 375 : sliderHeight) + 6}px` : `${(sliderHeight === 0 ? 375 : sliderHeight)}px` }}>
        {/* Previous Button */}
        <div
          ref={prevButtonRef}
          onClick={() => {
